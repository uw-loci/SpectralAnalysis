...
%         A(:,i-1,j+1)+A(:,i,j+1)+A(:,i+1,j+1);
        ...
        h=fwhm(1:length(g),g);%finding location of fwhm
        maxMatrix(i-bin,j-bin)=double(h)*12.5/noOfBin;
...
mask=maxMatrix;
mask=mask>0.2;
maskedPixel=mask.*maxMatrix;
totValue=sum(sum(maskedPixel));
averageValue=totValue/length(find(mask))
